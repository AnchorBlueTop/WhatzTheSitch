...
public struct PieChartRow : View {
    ...
    var colArray: [Color] = [Color("Magenta"), Color("Violet"), Color.red, Color.blue, Color.gray, Color.orange]
    var slices: [PieSlice] {
        ...
        GeometryReader { geometry in
            ZStack{
                ForEach(0..<self.slices.count){ i in
                    PieChartCell(rect: geometry.frame(in: .local), startDeg: self.slices[i].startDeg, endDeg: self.slices[i].endDeg, index: i, backgroundColor: self.backgroundColor,accentColor: self.colArray[i])
